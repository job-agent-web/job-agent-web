...
async function setPaneLock(supabaseUrl, serviceRoleKey, payload) {
  ...
  let lockedPanes = normalizePaneLockList(user && user.user_metadata && (user.user_metadata.locked_panes || user.user_metadata.lockedPanes));
  let unlockedPanes = normalizePaneLockList(user && user.user_metadata && (user.user_metadata.unlocked_panes || user.user_metadata.unlockedPanes));
  let response;
  if (!email) {
    return json(400, { ok: false, message: "Email is required." });
  }
